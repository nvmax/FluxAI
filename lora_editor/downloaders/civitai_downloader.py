...
class CivitAIDownloader:
    ...
    def download_file(self, url: str, dest_folder: str) -> str:
        ...
        try:
            ...
            # Start the download
            response = requests.get(url, headers=self.headers, stream=True, allow_redirects=False)
    ...
    def download(self, url: str, dest_folder: str, progress_callback=None) -> tuple:
        ...
        try:
            ...
            # If it's a model page URL, get the download URL and metadata
            if 'civitai.com/models/' in url:
                download_url, trigger_words, recommended_weight = self.get_download_url(url)
                filename = self.download_file(download_url, dest_folder)
                return filename, trigger_words, recommended_weight
            else:
                # Direct download URL
                filename = self.download_file(url, dest_folder)
                # No trigger words available for direct downloads
                return filename, "", 1.0
