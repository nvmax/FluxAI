...
class NavigationButtons(ttk.Frame):
    def __init__(self, parent, commands):
        ...
        # Create buttons with triangle symbols - direct command binding
        self.buttons['up_five'] = ttk.Button(
            self,
            text="▲▲",
            width=3,
            command=commands.get('up_five')
        )
        self.buttons['up_five'].pack(side=tk.TOP, pady=1)

        self.buttons['up'] = ttk.Button(
            self,
            text="▲",
            width=3,
            command=commands.get('up')
        )
        self.buttons['up'].pack(side=tk.TOP, pady=1)

        self.buttons['down'] = ttk.Button(
            self,
            text="▼",
            width=3,
            command=commands.get('down')
        )
        self.buttons['down'].pack(side=tk.TOP, pady=1)

        self.buttons['down_five'] = ttk.Button(
            self,
            text="▼▼",
            width=3,
            command=commands.get('down_five')
        )
        self.buttons['down_five'].pack(side=tk.TOP, pady=1)
